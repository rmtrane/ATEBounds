...
description: >
  See how we can use bivariate data to get a sense of what the bounds obtained from trivariate data could look like.
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Trivariate Bounds from Bivariate Data}
...
if(file.exists(here::here("vignettes_data/many_sample_joints.rds"))){
  ...
  many_sample_joints <- sim_probs %>%
    mutate(
      samp_joints = future_map(pot_covs, sample_joint_probs, return_bounds = TRUE, n = 1000, max_rejections = 500)
    )
...
plot <- for_plot %>% #select(k, center, facet_row, facet_col) %>% arrange(center) %>% unique() %>% View()
  ggplot(aes(x = id/max(id))) + 
    ...
    geom_text(data = for_plot %>% 
                ...
                unique(),
              aes(label = j, x = 0.25, y = if_else(bivariate_upper < 0.6,
                                                   0.85, -0.85))) +
    ylim(c(-1,1)) +
...
```{r}
chosen_ones <- c(53, 6, 66, #19,36, 94, 26
                 38, 7, 88,
                 73, 44, 60)
...
subset_plot <- ggplot(for_subset_plot,
  ...
  scale_color_manual(values = c("black", "grey50", "red4"),
                     #labels = c("Bivariate Bounds", "Overlaps Zero", "Does not Overlap Zero"),
                     breaks = c("Bivariate Bounds", "Overlaps Zero", "Does not Overlap Zero")) + 
  labs(x = "", y = "ATE",
       color = "",
       linetype = ""
  ) + 
...
* The first row shows us three scenarios with relatively wide bivariate bounds, more or less centered around zero. Yet we would draw three different conclusions:
    * `r chosen_ones[1]`: even with trivariate data, we would not be able to determine direction
    * `r chosen_ones[2]`: with trivariate data, we would not be able to determine direction if the true ACE is negative. About `r filter(subset_plot_summaries, row_i == 1, col_j == 2)[["p_no_zero"]]*100`% of the joint probabilities would result in the trivariate data giving us the direction, and in all of these cases the direction is positive. 
    * `r chosen_ones[3]`: with trivariate data, we would be able to determine direction with about `r filter(subset_plot_summaries, J == chosen_ones[3])[["p_no_zero"]]*100`% of the possible joint distributions, but the direction could go either way. 
...
* The second row shows scenarios where the bounds are relatively wide and centered relatively far from zero (here, it is above, but you can similarly find cases where it would be below). Again, three difference conclusions:
    * `r chosen_ones[4]`: even with trivariate data, would would not be able to determine direction
    ...
    * Notice the big difference in information in the bivariate data between `r chosen_ones[5]` and `r chosen_ones[6]` even though the bivariate bounds are relatively similar. One tells us that it is pretty likely that trivariate data would allow us to conclude the direction of the effect is positive, while the other one tells us it is rather unlikely we would be able to determine direction based on trivariate data. 
    
* Finally, the third row shows us three scenarios where the bivariate bounds are relatively narrow and centered around zero. Once again, we would arrive at three different conclusions:
    * `r chosen_ones[7]`: trivariate data would not help us determine direction
    * `r chosen_ones[8]`: we would be able to determine direction from trivariate data in `r filter(subset_plot_summaries, J == chosen_ones[8])[["p_no_zero"]]*100`% of the drawn joints, but from the information in the bivariate data, we cannot rule out either direction. This seems rather surprising to me: even though the bivariate bounds are pretty tight around zero, bounds from trivariate data could still end up entirely on either side of zero!
...
Save plot and a few objects for later use.

```{r}
write_rds(x = subset_plot_summaries,
          path = here::here("vignettes_data", "subset_plot_summaries.Rds"))
...
if(file.exists(here::here("vignettes_data/many_sample_joints_mono.rds"))){
  ...
  many_sample_joints_mono <- sim_probs_mono %>%
    mutate(
      samp_joints = future_map(pot_covs, sample_joint_probs, x, return_bounds = TRUE, n = 1000, max_rejections = 100,
                               x_mono = TRUE)
    )
