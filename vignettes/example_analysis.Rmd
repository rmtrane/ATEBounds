...
summaries_for_plots <- smoking_depression_w_intercept %>% 
  ungroup() %>% 
  select(SNP, regression, beta, starts_with("P(Z ="), p_outcome, beta0, `thetas/gammas`) %>% 
  unnest_wider(`thetas/gammas`) %>% 
  rename(xy_given_z0 = ...1,
         xy_given_z1 = ...2,
         xy_given_z2 = ...3)

write_csv(
  summaries_for_plots,
  here::here("vignettes_data/summary_stats_smoking_on_depression.csv")
)
...
ggsave(
  ...
  filename = here::here("figures/example_analyses", 
                        "smoking_depression_marginal_Z.png"),
  dpi = 300, width = 5, height = 2
)


depression_coefficients <- summaries_for_plots %>% 
...
ggsave(
  plot = depression_coefficients,
  filename = here::here("figures/example_analyses", 
                        "smoking_depression_coefficients.png"),
...
First, the marginal probabilites $P(Z = z), z = 0,1,2$. 
...
smoking_depression_individual_SNPs_plot <- smoking_depression_trivariate_bounds %>% 
  ...
  mutate(id = row_number()#,
         #contains_zero = as.character(as.numeric(contains_zero)) #if_else(contains_zero, "Overlaps Zero", "Does not Overlap Zero")
         ) %>% 
  ungroup() %>% 
...
## Subset of above plot
smoking_depression_individual_SNPs_plot_subset <- smoking_depression_trivariate_bounds %>% 
  filter(SNP %in% unique(SNP)[c(2, 5, 17, 25, 34, 52, 65, 78)]) %>% 
  group_by(SNP) %>% 
  arrange(lower) %>% 
  mutate(id = row_number()#,
         #contains_zero = as.character(as.numeric(contains_zero)) #if_else(contains_zero, "Overlaps Zero", "Does not Overlap Zero")
...
# smoking_depression_individual_SNPs_plot_subset + 
#     labs(title = "Possible Trivariate Bounds from Bivariate Data",
#          subtitle = paste0("Exposure: Smoking (id: ", smoking_experiment$id, "). ",
#                            "Outcome: Depression (id: ", depression_experiment$id, ")."))

ggsave(
  plot = smoking_depression_individual_SNPs_plot_subset,
  filename = here::here("figures/example_analyses", 
                        paste0("smoking_depression_individual_SNPs_plot_subset_", smoking_experiment$id, "_", depression_experiment$id,".png")),
...
```{r}
summaries_for_plots_lung_cancer_3 <- smoking_lung_cancer_3_w_intercept %>% 
  ungroup() %>% 
  select(SNP, regression, beta, starts_with("P(Z ="), p_outcome, beta0, `thetas/gammas`) %>% 
  unnest_wider(`thetas/gammas`) %>% 
  rename(xy_given_z0 = ...1,
...
write_csv(
  summaries_for_plots_lung_cancer_3,
  here::here("vignettes_data/summary_stats_smoking_on_lung_cancer_3.csv")
)


marginal_Z_lung_cancer_3 <- summaries_for_plots_lung_cancer_3 %>% 
  filter(regression == "exposure") %>% 
  pivot_longer(cols = starts_with("P(Z ="),
               names_to = "z", values_to = "p") %>% 
  ggplot(aes(x = p)) +
...
ggsave(
  plot = marginal_Z_lung_cancer_3,
  filename = here::here("figures/example_analyses", 
                        "smoking_lung_cancer_3_marginal_Z.png"),
  dpi = 300, width = 5, height = 2
)


lung_cancer_3_coefficients <- summaries_for_plots_lung_cancer_3 %>% 
  pivot_longer(cols = c(beta, beta0),
               names_to = "coef", values_to = "value") %>% 
  mutate(real_coefs = case_when(coef == "beta" & regression == "exposure" ~ "beta[1]", 
                                coef == "beta0" & regression == "exposure" ~ "beta[0]",
...
ggsave(
  plot = lung_cancer_3_coefficients,
  filename = here::here("figures/example_analyses", 
                        "smoking_lung_cancer_3_coefficients.png"),
  dpi = 300, width = 5, height = 3
)
```
...
smoking_lung_cancer_3_strength_summaries <- smoking_lung_cancer_3_bounds %>% 
  mutate(strength = map_dbl(thetas, ~max(abs(outer(.x, .x, "-"))))) %>% 
  summarize(
    min = min(strength),
    max = max(strength),
    mean = mean(strength)
...
write_csv(smoking_lung_cancer_3_strength_summaries,
          file = here("vignettes_data/smoking_lung_cancer_3_strength_summaries.csv"))

smoking_lung_cancer_3_strength_histogram <- smoking_lung_cancer_3_bounds %>% 
  mutate(strength = map_dbl(thetas, ~max(abs(outer(.x, .x, "-"))))) %>% 
  ggplot(aes(x = strength)) +
    geom_histogram(binwidth = 0.001, boundary = 0) + 
    scale_x_continuous(limits = c(0, 0.012), 
...
ggsave(
  smoking_lung_cancer_3_strength_histogram,
  filename = here::here("figures/example_analyses",
                        "smoking_lung_cancer_3_strength_histogram.png"),
  dpi = 300, width = 8, height = 4
)
...
smoking_lung_cancer_3_individual_SNPs_plot_subset <- smoking_lung_cancer_3_trivariate_bounds %>% 
  filter(SNP %in% unique(SNP)[c(2, 5, 17, 25, 34, 52, 65, 78)]) %>% 
  group_by(SNP) %>% 
  arrange(lower) %>% 
  mutate(id = row_number()) %>% 
...
ggsave(
  plot = smoking_lung_cancer_3_individual_SNPs_plot_subset,
  filename = here::here("figures/example_analyses", 
                        paste0("smoking_lung_cancer_3_individual_SNPs_plot_subset_", smoking_experiment$id, "_", lung_cancer_3_experiment$id,".png")),
  width = 8, height = 6.75, dpi = 300
)
...
```{r}
obesity_experiment <- ao %>% filter(id == "ieu-a-91")

obesity_instruments <- extract_instruments(outcomes = "ieu-a-91")

depression_experiment <- ao %>% filter(id == "ukb-d-20544_11")
depression_data <- extract_outcome_data(snps = obesity_instruments$SNP, outcomes = depression_experiment$id)

obesity_depression_harmonized <- harmonise_data(obesity_instruments, depression_data)# %>% as_tibble()
```


```{r}
obesity_depression_pop_probs <- tibble(regression = c("outcome", "exposure"),
                                       p_outcome = c(depression_experiment  %$% { ncase / (ncase + ncontrol) },
                                                     obesity_experiment %$% { ncase / (ncase + ncontrol) }))

obesity_depression_prep_for_bounds <- obesity_depression_harmonized %>%
  rowwise() %>% 
  mutate(samplesize.exposure = obesity_experiment$ncase + obesity_experiment$ncontrol,
         samplesize.outcome = depression_experiment$ncase + depression_experiment$ncontrol,
         ave_eaf = weighted.mean(x = c_across(c(eaf.exposure, eaf.outcome)),
                                 w = c_across(c(samplesize.exposure, samplesize.outcome)))) %>% 
  select(SNP, contains("beta"), ave_eaf) %>% 
...
```{r}
obesity_depression_w_intercept <- obesity_depression_prep_for_bounds %>% 
  left_join(obesity_depression_pop_probs) %>% 
  rowwise() %>% 
  mutate(find_intercept = list(find_intercept(beta1 = beta,
                                              p_outcome = p_outcome,
                                              pz = c(`P(Z = 0)`, `P(Z = 1)`, `P(Z = 2)`),
...
obesity_depression_bounds <- obesity_depression_w_intercept %>% 
  select(-find_intercept, -beta0, -beta, -p_outcome, -ave_eaf) %>% 
  mutate(regression = if_else(regression == "outcome", "gammas", "thetas")) %>% 
  pivot_wider(names_from = regression, values_from = `thetas/gammas`) %>% 
  rowwise() %>% 
...
```{r}
obesity_depression_bivariate_bounds <- obesity_depression_bounds %>% 
  arrange(lower) %>% 
  ggplot(aes(x = SNP)) +
    geom_errorbar(aes(ymax = upper, ymin = lower)) +
    theme_bw() +
...
obesity_depression_bivariate_bounds +
      labs(title = "Bivariate bounds",
         subtitle = paste0("Exposure: Smoking (id: ", obesity_experiment$id, "). ",
                           "Outcome: Depression (id: ", depression_experiment$id, ")."))

ggsave(
  plot = obesity_depression_bivariate_bounds,
  filename = here::here("figures/example_analyses", 
                        paste0("obesity_depression_bivaraite_bounds_", obesity_experiment$id, "_", depression_experiment$id,".png")),
  width = 8, height = 6, dpi = 300
)
```
...
```{r}
obesity_depression_filename <- paste0("samples_of_joints_", obesity_experiment$id, "_", depression_experiment$id, ".Rds")

if(file.exists(here::here("vignettes_data/example_analyses", obesity_depression_filename))){
  obesity_depression_samples_of_joints <- read_rds(here::here("vignettes_data/example_analyses", obesity_depression_filename))
} else {
  set.seed(3843173)
  
  obesity_depression_samples_of_joints <- obesity_depression_bounds %>% 
    rowwise() %>% 
    mutate(pot_covs = list(potential_covs(thetas = thetas, gammas = gammas)),
           sample_joint = list(sample_joint_probs(pot_covs, n = 500, return_bounds = TRUE))) %>% 
    ungroup()
  
  write_rds(obesity_depression_samples_of_joints, here::here("vignettes_data/example_analyses", obesity_depression_filename))
}

obesity_depression_trivariate_bounds <- obesity_depression_samples_of_joints %>% 
  select(SNP, bivariate_lower = lower, bivariate_upper = upper, sample_joint) %>% 
  unnest(sample_joint) %>% 
  unnest(joint) %>% 
  unnest_wider(bounds) %>% 
...
```{r fig.width = 8}
obesity_depression_individual_SNPs_plot <- obesity_depression_trivariate_bounds %>% 
  group_by(SNP) %>% 
  arrange(lower) %>% 
  mutate(id = row_number()#,
         #contains_zero = as.character(as.numeric(contains_zero)) #if_else(contains_zero, "Overlaps Zero", "Does not Overlap Zero")
...
obesity_depression_individual_SNPs_plot + 
    labs(title = "Possible Trivariate Bounds from Bivariate Data",
         subtitle = paste0("Exposure: Obesity (id: ", obesity_experiment$id, "). ",
                           "Outcome: Depression (id: ", depression_experiment$id, ")."))

ggsave(
  plot = obesity_depression_individual_SNPs_plot,
  filename = here::here("figures/example_analyses", 
                        paste0("obesity_depression_individual_SNPs_plot_", obesity_experiment$id, "_", depression_experiment$id,".png")),
  width = 11, height = 6.5, dpi = 300
)
```
...
```{r}
lipo_id <- "finn-a-E4_LIPOPROT" 
stroke_id <- "ebi-a-GCST006908"
chd_id <- "ukb-d-I9_CHD"
vitamin_id <- "ukb-a-462"
```



```{r}
lipo_experiment <- ao %>% filter(id == lipo_id)

lipo_instruments <- extract_instruments(outcomes = "ieu-a-91")
...
# Cholesterol on Heart Attack

## Collect Data

```{r}
cholesterol <- "ukb-a-108"
heart_attack <- "ukb-a-434"

cholesterol_experiment <- ao %>% filter(id == cholesterol) 
cholesterol_instruments <- extract_instruments(cholesterol_experiment$id)

heart_attack_experiment <- ao %>% filter(id == heart_attack)
...
```{r}
cholesterol_heart_attack_strength_summaries <- cholesterol_heart_attack_w_intercept %>% 
  filter(regression == "exposure") %>% 
  rowwise() %>% 
  mutate(strength = max(abs(outer(`thetas/gammas`, `thetas/gammas`, "-")))) %>% 
  ungroup() %>% 
  summarize(
    min = min(strength),
    max = max(strength),
...
write_csv(cholesterol_heart_attack_strength_summaries,
          file = here::here("vignettes_data/cholesterol_heart_attack_strength_summaries.csv"))

cholesterol_heart_attack_strength_histogram <- cholesterol_heart_attack_w_intercept %>% 
  filter(regression == "exposure") %>% 
  rowwise() %>% 
  mutate(strength = max(abs(outer(`thetas/gammas`, `thetas/gammas`, "-")))) %>% 
  ggplot(aes(x = strength)) +
    geom_histogram(binwidth = 0.00025, boundary = 0) + 
    scale_x_continuous(limits = c(0, 0.0025), 
                       breaks = seq(0, 0.0025, by = 0.0005)) +
    labs(x = "Strength", y = "Count") +
    theme_bw()

ggsave(
  cholesterol_heart_attack_strength_histogram,
  filename = here::here("figures/example_analyses",
                        "cholesterol_heart_attack_strength_histogram.png"),
  dpi = 300, width = 8, height = 4
)

summaries_for_plots_cholesterol <- cholesterol_heart_attack_w_intercept %>% 
  ungroup() %>% 
  select(SNP, regression, beta, starts_with("P(Z ="), p_outcome, beta0, `thetas/gammas`) %>% 
  unnest_wider(`thetas/gammas`) %>% 
  rename(xy_given_z0 = ...1,
...
write_csv(
  summaries_for_plots_cholesterol,
  here::here("vignettes_data/summary_stats_cholesterol_on_heart_attack.csv")
)


marginal_Z_cholesterol <- summaries_for_plots_cholesterol %>% 
  filter(regression == "exposure") %>% 
  pivot_longer(cols = starts_with("P(Z ="),
               names_to = "z", values_to = "p") %>% 
  ggplot(aes(x = p)) +
...
ggsave(
  plot = marginal_Z_cholesterol,
  filename = here::here("figures/example_analyses", 
                        "cholesterol_heart_attack_marginal_Z.png"),
  dpi = 300, width = 5, height = 2
)

cholesterol_heart_attack_marginal_conditionals <- summaries_for_plots_cholesterol %>% 
  pivot_longer(cols = starts_with("xy_given_z"),
               names_to = "vars", values_to = "p") %>% 
  mutate(regression = if_else(regression == "exposure",
                              "P(X = 1 | Z = z)", "P(Y = 1 | Z = z)"),
...
cholesterol_coefficients <- summaries_for_plots_cholesterol %>% 
  pivot_longer(cols = c(beta, beta0),
               names_to = "coef", values_to = "value") %>% 
  mutate(real_coefs = case_when(coef == "beta" & regression == "exposure" ~ "beta[1]", 
                                coef == "beta0" & regression == "exposure" ~ "beta[0]",
                                ...
                                coef == "beta0" & regression == "outcome" ~ "gamma[0]")) %>% 
  ggplot(aes(x = value)) +
    geom_histogram(boundary = 0, bins = 15, 
                   color = "black") +
    facet_wrap(~ real_coefs,
               scales = "free",
               labeller = label_parsed) +
...
cholesterol_heart_attack_bivariate_bounds + 
    labs(title = "Bivariate bounds",
         subtitle = paste0("Exposure: Cholesterol (id: ", cholesterol_experiment$id, "). ",
                           "Outcome: Heart Attack (id: ", heart_attack_experiment$id, ")."))

ggsave(
  plot = cholesterol_heart_attack_bivariate_bounds,
  filename = here::here("figures/example_analyses", 
                        paste0("cholesterol_heart_attack_bivaraite_bounds_", cholesterol_experiment$id, "_", heart_attack_experiment$id,".png")), 
  dpi = 300, width = 8, height = 10
)
```
...
```{r}
cholesterol_heart_attack_filename <- paste0("samples_of_joints_", 
                                            cholesterol_experiment$id, "_", heart_attack_experiment$id, ".Rds")

if(file.exists(here::here("vignettes_data/example_analyses", cholesterol_heart_attack_filename))){
  cholesterol_heart_attack_samples_of_joints <- read_rds(here::here("vignettes_data/example_analyses",
                                                                    cholesterol_heart_attack_filename))
} else {
  set.seed(7892312)
  
  cholesterol_heart_attack_samples_of_joints <- cholesterol_heart_attack_bounds %>% 
    mutate(J = paste(row_number(), "of", n())) %>% # To be printed so we can follow the process
    rowwise() %>% 
    mutate(pot_covs = list(potential_covs(thetas = thetas, gammas = gammas)),
           sample_joint = list(sample_joint_probs(pot_covs, n = 500, return_bounds = TRUE,
                                                  print_as_progress = J, print_progress = FALSE))) %>% 
    ungroup() %>% 
    select(-J)
  
  write_rds(cholesterol_heart_attack_samples_of_joints, 
            here::here("vignettes_data/example_analyses", 
                       cholesterol_heart_attack_filename))
}
...
cholesterol_heart_attack_individual_SNPs_plot_subset <- cholesterol_heart_attack_trivariate_bounds %>% 
  filter(SNP %in% unique(SNP)[c(2, 5, 10, 17, 25, 34, 45, 52)]) %>% 
  group_by(SNP) %>% 
  arrange(lower) %>% 
  mutate(id = row_number()) %>% 
  ungroup() %>% 
...
ggsave(
  ...
  filename = here::here("figures/example_analyses", 
                        paste0("cholesterol_heart_attack_individual_SNPs_plot_subset_", 
                               cholesterol_experiment$id, "_", heart_attack_experiment$id,".png")),
  width = 8, height = 6.75, dpi = 300
)
...
ggsave(
  ...
  filename = here::here("figures/example_analyses", 
                        paste0("cholesterol_heart_attack_intersection_bounds_plot_", 
                               cholesterol_experiment$id, "_", heart_attack_experiment$id,".png")),
  width = 8, height = 4, dpi = 300
)
```
...
obesity_experiment <- ao %>% filter(id == obesity) 
obesity_instruments <- extract_instruments(obesity_experiment$id)

heart_attack_experiment <- ao %>% filter(id == heart_attack)

heart_attack_data <- extract_outcome_data(snps = obesity_instruments$SNP, outcomes = heart_attack_experiment$id)

obesity_heart_attack_harmonized <- harmonise_data(obesity_instruments, heart_attack_data) %>% as_tibble()
```

## Summarize to find probabilities

```{r}
obesity_heart_attack_pop_probs <- tibble(regression = c("outcome", "exposure"),
                                             p_outcome = c(obesity_experiment  %$% { ncase / (ncase + ncontrol) },
                                                           heart_attack_experiment %$% { ncase / (ncase + ncontrol) }))

obesity_heart_attack_prep_for_bounds <- obesity_heart_attack_harmonized %>%
  rowwise() %>% 
  mutate(samplesize.exposure = obesity_experiment$ncase + obesity_experiment$ncontrol,
         samplesize.outcome = heart_attack_experiment$ncase + heart_attack_experiment$ncontrol,
         ave_eaf = weighted.mean(x = c_across(c(eaf.exposure, eaf.outcome)),
...
obesity_heart_attack_w_intercept <- obesity_heart_attack_prep_for_bounds %>% 
  left_join(obesity_heart_attack_pop_probs) %>% 
  rowwise() %>% 
  mutate(find_intercept = list(find_intercept(beta1 = beta,
                                              p_outcome = p_outcome,
                                              pz = c(`P(Z = 0)`, `P(Z = 1)`, `P(Z = 2)`),
...
```{r}
obesity_heart_attack_strength_summaries <- obesity_heart_attack_w_intercept %>% 
  filter(regression == "exposure") %>% 
  rowwise() %>% 
  mutate(strength = max(abs(outer(`thetas/gammas`, `thetas/gammas`, "-")))) %>% 
  ungroup() %>% 
...
write_csv(obesity_heart_attack_strength_summaries,
          file = here::here("vignettes_data/obesity_heart_attack_strength_summaries.csv"))

obesity_heart_attack_strength_histogram <- obesity_heart_attack_w_intercept %>% 
  filter(regression == "exposure") %>% 
  rowwise() %>% 
  mutate(strength = max(abs(outer(`thetas/gammas`, `thetas/gammas`, "-")))) %>% 
  ggplot(aes(x = strength)) +
...
ggsave(
  obesity_heart_attack_strength_histogram,
  filename = here::here("figures/example_analyses",
                        "obesity_heart_attack_strength_histogram.png"),
  dpi = 300, width = 8, height = 4
)

summaries_for_plots_obesity <- obesity_heart_attack_w_intercept %>% 
  ungroup() %>% 
  select(SNP, regression, beta, starts_with("P(Z ="), p_outcome, beta0, `thetas/gammas`) %>% 
  unnest_wider(`thetas/gammas`) %>% 
  rename(xy_given_z0 = ...1,
...
write_csv(
  summaries_for_plots_obesity,
  here::here("vignettes_data/summary_stats_obesity_on_heart_attack.csv")
)


marginal_Z_obesity <- summaries_for_plots_obesity %>% 
  filter(regression == "exposure") %>% 
  pivot_longer(cols = starts_with("P(Z ="),
               names_to = "z", values_to = "p") %>% 
  ggplot(aes(x = p)) +
...
ggsave(
  plot = marginal_Z_obesity,
  filename = here::here("figures/example_analyses", 
                        "obesity_heart_attack_marginal_Z.png"),
  dpi = 300, width = 5, height = 2
)

obesity_heart_attack_marginal_conditionals <- summaries_for_plots_obesity %>% 
  pivot_longer(cols = starts_with("xy_given_z"),
               names_to = "vars", values_to = "p") %>% 
  mutate(regression = if_else(regression == "exposure",
                              "P(X = 1 | Z = z)", "P(Y = 1 | Z = z)"),
...
ggsave(
  plot = obesity_heart_attack_marginal_conditionals,
  filename = here::here("figures/example_analyses",
                        "obesity_heart_attack_marginal_conditionals.png"),
  dpi = 300, width = 5, height = 3
)


obesity_coefficients <- summaries_for_plots_obesity %>% 
  pivot_longer(cols = c(beta, beta0),
               names_to = "coef", values_to = "value") %>% 
  mutate(real_coefs = case_when(coef == "beta" & regression == "exposure" ~ "beta[1]", 
                                coef == "beta0" & regression == "exposure" ~ "beta[0]",
...
ggsave(
  plot = obesity_coefficients,
  filename = here::here("figures/example_analyses", 
                        "obesity_heart_attack_coefficients.png"),
  dpi = 300, width = 5, height = 3
)
```
...
```{r}
obesity_heart_attack_bounds <- obesity_heart_attack_w_intercept %>% 
  select(-find_intercept, -beta0, -beta, -p_outcome, -ave_eaf) %>% 
  mutate(regression = if_else(regression == "outcome", "gammas", "thetas")) %>% 
  pivot_wider(names_from = regression, values_from = `thetas/gammas`) %>% 
  rowwise() %>% 
...
obesity_heart_attack_bivariate_bounds <- obesity_heart_attack_bounds %>% 
  arrange(lower) %>% 
  ggplot(aes(y = SNP)) +
    geom_errorbar(aes(xmax = upper, xmin = lower)) +
    theme_bw() +
...
obesity_heart_attack_bivariate_bounds + 
    labs(title = "Bivariate bounds",
         subtitle = paste0("Exposure: obesity (id: ", obesity_experiment$id, "). ",
                           "Outcome: Heart Attack (id: ", heart_attack_experiment$id, ")."))

ggsave(
  plot = obesity_heart_attack_bivariate_bounds,
  filename = here::here("figures/example_analyses", 
                        paste0("obesity_heart_attack_bivaraite_bounds_", obesity_experiment$id, "_", heart_attack_experiment$id,".png")), 
  dpi = 300, width = 8, height = 10
)
```
...
```{r}
obesity_heart_attack_filename <- paste0("samples_of_joints_", 
                                            obesity_experiment$id, "_", heart_attack_experiment$id, ".Rds")

if(file.exists(here::here("vignettes_data/example_analyses", obesity_heart_attack_filename))){
  obesity_heart_attack_samples_of_joints <- read_rds(here::here("vignettes_data/example_analyses",
                                                                    obesity_heart_attack_filename))
} else {
  set.seed(7892312)
  
  obesity_heart_attack_samples_of_joints <- obesity_heart_attack_bounds %>% 
    mutate(J = paste(row_number(), "of", n())) %>% # To be printed so we can follow the process
    rowwise() %>% 
    mutate(pot_covs = list(potential_covs(thetas = thetas, gammas = gammas)),
           sample_joint = list(sample_joint_probs(pot_covs, n = 500, return_bounds = TRUE,
  ...
  write_rds(obesity_heart_attack_samples_of_joints, 
            here::here("vignettes_data/example_analyses", 
                       obesity_heart_attack_filename))
}

obesity_heart_attack_trivariate_bounds <- obesity_heart_attack_samples_of_joints %>% 
  select(SNP, bivariate_lower = lower, bivariate_upper = upper, sample_joint) %>% 
  unnest(sample_joint) %>% 
  unnest(joint) %>% 
  unnest_wider(bounds) %>% 
...
obesity_heart_attack_individual_SNPs_plot <- obesity_heart_attack_trivariate_bounds %>% 
  group_by(SNP) %>% 
  arrange(lower) %>% 
  mutate(id = row_number()) %>% 
  ungroup() %>% 
...
ggsave(
  plot = obesity_heart_attack_individual_SNPs_plot,
  filename = here::here("figures/example_analyses", 
                        paste0("obesity_heart_attack_individual_SNPs_plot_", obesity_experiment$id, "_", heart_attack_experiment$id,".png")),
  width = 8, height = 9, dpi = 300
)

obesity_heart_attack_individual_SNPs_plot_subset <- obesity_heart_attack_trivariate_bounds %>% 
  filter(SNP %in% unique(SNP)[c(2, 5, 10, 17, 25, 34, 45, 52)]) %>% 
  group_by(SNP) %>% 
  arrange(lower) %>% 
  mutate(id = row_number()) %>% 
...
ggsave(
  plot = obesity_heart_attack_individual_SNPs_plot_subset,
  filename = here::here("figures/example_analyses", 
                        paste0("obesity_heart_attack_individual_SNPs_plot_subset_", 
                               obesity_experiment$id, "_", heart_attack_experiment$id,".png")),
  width = 8, height = 6.75, dpi = 300
)


obesity_heart_attack_individual_SNPs_plot +
    labs(title = "Possible Trivariate Bounds from Bivariate Data",
         subtitle = paste0("Exposure: obesity (id: ", obesity_experiment$id, "). ",
                           "Outcome: Heart Attack (id: ", heart_attack_experiment$id, ")."))
```
...
```{r}
obesity_heart_attack_pairs_of_snps <- expand_grid(SNP1 = unique(obesity_heart_attack_trivariate_bounds$SNP),
                                                      SNP2 = unique(obesity_heart_attack_trivariate_bounds$SNP)) %>% 
  filter(SNP1 != SNP2) %>% 
  rowwise() %>% 
  summarize(SNPs = list(sort(c(SNP1, SNP2)))) %>% 
  filter(duplicated(SNPs)) %>% 
...
obesity_heart_attack_chosen_pairs <- obesity_heart_attack_pairs_of_snps %>% 
  sample_n(8)

pander::pander(obesity_heart_attack_chosen_pairs)

obesity_heart_attack_both_bounds <- obesity_heart_attack_chosen_pairs %>% 
  rowwise() %>% 
  mutate(
    SNP1_bounds = list(
      obesity_heart_attack_trivariate_bounds %>% 
        filter(SNP == SNP1) %>% 
        select(SNP1_bivariate_lower = bivariate_lower, SNP1_bivariate_upper = bivariate_upper, 
               SNP1_lower = lower, SNP1_upper = upper)
    ),
...
obesity_heart_attack_both_bounds

obesity_heart_attack_intersection_bounds <- obesity_heart_attack_both_bounds %>% 
  rowwise() %>% 
  mutate(intersection_lower = max(SNP1_lower, SNP2_lower),
         intersection_upper = min(SNP1_upper, SNP2_upper),
         bivariate_intersection_lower = max(SNP1_bivariate_lower, SNP2_bivariate_lower),
...
```{r fig.height = 14, fig.width = 10}
obesity_heart_attack_intersection_bounds_plot <- obesity_heart_attack_intersection_bounds %>% 
  group_by(SNP1, SNP2) %>% 
  arrange(intersection_lower) %>% 
  mutate(i = row_number()) %>% 
  ungroup() %>% 
...
obesity_heart_attack_intersection_bounds_plot + 
    labs(title = "Intersections of Possible Trivariate Bounds from Bivariate Data",
         subtitle = paste0("Exposure: obesity (id: ", obesity_experiment$id, "). ",
                           "Outcome: Heart Attack (id: ", heart_attack_experiment$id, ")."))
```
...
```{r}
ggsave(
  plot = obesity_heart_attack_intersection_bounds_plot,
  filename = here::here("figures/example_analyses", 
                        paste0("obesity_heart_attack_intersection_bounds_plot_", 
                               obesity_experiment$id, "_", heart_attack_experiment$id,".png")),
  width = 8, height = 4, dpi = 300
)
```
...
ggsave(
  ...
  filename = here::here("figures/example_analyses",
                        "smoking_depression_marginal_conditionals.png"),
  dpi = 300, width = 5, height = 3
)
