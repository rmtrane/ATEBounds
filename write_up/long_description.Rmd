...
Overall, in the context of two-sample MR studies with summary statistics, most genetic instruments are weak, which means that the chances that bivariate bounds from MR analyses are informative with width less than 1 are very slim. In particular, a strength requirement of $\text{ST} > 0.5$ to guarantee a bound with length less than $1$ is a tall order in many MR studies. To illustrate this point, we simulate data from a model very similar to the one underlying most GWAS analyses, on which many two-sample MR studies build. Specifically, for each observation we draw a value of our instrument $Z$ such that $P(Z = 0) = P(Z = 2) = 0.25$ and $P(Z = 1) = 0.5$, and a value of an unmeasured confounder $U$ from the standard normal. The exposure $X$ is binary with $P(X = 1 | Z = z, U = u) = \text{logit}(\beta_0 + \beta_Z\cdot z + \beta_U \cdot u)$, and the outcome $Y$ is binary with $P(Y = 1 | X = x, U = u) = \text{logit}(\gamma_0 + \gamma_X \cdot x + \beta_U \cdot u)$. We simulate 10,000,000 observations from this model for each combination of $\beta_Z \in \{1, 1.5, 1.75, 1.9, 2, 2.1, 2.25, 2.5, 3, 3.5, 5\}$, $\beta_U \in \{0.1, 0.5\}$, and $\gamma_X \in \{1, 1.5, 2\}$. In each simulation, we assume a "best case scenario" where the intercepts are such that the conditional probabilities $P(Y = 1 | X = x, U)$ and $P(X = 1 | Z = z, U)$ are as spread out as possible. To this point, we set $\beta_0 = -\beta_Z$ and $\gamma_0 = -\gamma_X/2$. 

Figure \ref{fig:coef_vs_strength} shows the relationship between the values of $\beta_Z$ and the strength of the IV. Extrapolating from the points, we see that to achieve a strength of 0.5, a $\beta_Z$ of more than $1.16$ is required if $\beta_U = 0.5$, while the threshold is $1.1$ if $\beta_U = 0.1$. We can also use this model to suggest the kind of IVs needed to be able to detect direction of the ATE for different sizes of the treatment effects. Figure \ref{fig:power_curves} shows the smallest values of $\beta_Z$ that lead to positive lower bound. This is very well in line with our previous observations -- very large coefficients are need for us to be able to detect direction of the average treatment effect using two-sample nonparametric bounds.

\begin{figure*}
  \centering
  \begin{subfigure}[t]{0.5\textwidth}
    \includegraphics[width = \textwidth]{`r here::here("figures/MR_coefs_vs_strength.png")`}
    \caption{Relationship between coefficients $\beta_Z$ and strength of the IV.}
    \label{fig:coef_vs_strength}
  \end{subfigure}%
  ~
  \begin{subfigure}[t]{0.5\textwidth}
    \includegraphics[width=\textwidth]{`r here::here("figures/power_curves.png")`}
