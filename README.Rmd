...
You can install this package using `remotes::install_github("rmtrane/ATEBounds")`. This site includes an introduction to finding nonparametric bounds on the ATE using this package ([Get started](articles/ATEBounds.html)) and documentation of the functions included. On top of that, there are a few pages with information on how to reproduce our results; see below for more. One that might be of particular interest is `vignette("example_analysis")`, which shows how to use this package to obtain two-sample bounds based on studies from the MR-Base database. 

By default, this package will let you find one-sample and two-sample bounds for instruments with 2, 3, or 4 categories. However, it also provides a few helper functions that allows you to use polymake to find bounds for instruments with any number of categories. See `vignette("polymake_example")` for details. 
...
- **Section 5** demonstrates our findings on two real MR studies. `vignette("example_analysis")` shows how we obtained the data and preprocessed it using the `TwoSampleMR` R-package [@mrbase], and how Figures 4 and 5 were created. It also includes the code needed to reproduce eFigures and eTables in eAppendix A.7.
