...
plot <- for_plot %>% #select(k, center, facet_row, facet_col) %>% arrange(center) %>% unique() %>% View()
  ggplot(aes(y = id/max(id))) + 
    geom_vline(aes(xintercept = bivariate_lower, color = "Bivariate Bounds")) + 
    geom_vline(aes(xintercept = bivariate_upper, color = "Bivariate Bounds")) + 
...
subset_plot <- ggplot(for_subset_plot,
  ...
  geom_rect(data = data.frame(x_min = rep(0.2, 3),
                              ...
                              y_min = rep(0.1, 3),
                              y_max = rep(0.2, 3),
                              contains_zero = c("Bivariate Bounds", "Overlaps Zero", "Does not Overlap Zero")),
            inherit.aes = FALSE,
            aes(xmin = x_min,
                xmax = x_max,
                ymin = y_min,
