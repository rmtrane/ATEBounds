...
Prior section revealed that bounds from two-sample MR studies require a strong instrument to guarantee informative bounds. However, it did not address whether the bound can become more informative if multiple valid instruments are available. In this section, we consider one arguably optimistic approach to aggregate bounds across multiple instruments and show that even in this optimistic scenario, bounds generally do not become more informative as the number of instruments increase.

Formally, consider the following logistic model, an extension of the model presented in the previous section, for the outcome when there are multiple instruments

\[
\begin{aligned}
\text{logit}(P(X = 1 | Z_1 = z_1, ..., Z_p = z_p, U = u)) &= \beta_0 + \sum_i \beta_i z_i + \beta_U u \\
\text{logit}(P(Y = 1 | X = x, U = u)) &= \gamma_0 + \gamma_1 x + \gamma_U u,
\end{aligned}
\]
...
We set \(P(Z_j = 0) = P(Z_j = 2) = 0.25\) and \(P(Z_j = 1) = 0.5\), and draw \(U\) from a standard normal distribution. We fix the coefficient for \(U\) to be the same in both expressions, \(\beta_U = \gamma_U \in \{0.1, 0.5\}\), and vary \(\gamma_1 \in \{1, 1.5, 2\}\). We generate \(\beta_i\) in two scenarios. In the first, we draw \(\beta_i\) i.i.d. from an uniform distribution with support \(0\) to \(0.1\). This is similar to the coefficiencts seen in MR analyses. In the second scenario, we draw \(\beta_i\) i.i.d. from an uniform distribution with support \(1\) to \(4\), similar to the range used for the power analysis in the previous section. For each combination of coefficients and support of the uniform distribution, we simulate \(10,000,000\) observations for each of \(p \in \{3, 10, 50\}\).
