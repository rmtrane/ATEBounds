...
To get bounds on $\alpha$, one simply provides values of $P(X = 1 | Z = z)$ (`theta`) and $P(Y = 1 | Z = z)$ (`gamma`) for two-sample bounds, or $P(X = x, Y = y | Z = z)$ (`zeta`) for one-sample bounds to the function `get_bounds()`. Let's generate some random values of $\theta$ and $\gamma$ for a three-leveled IV:
