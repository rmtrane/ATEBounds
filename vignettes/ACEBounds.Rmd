...
```{r}
print(sim_dat$simulated_data, n = 10)
```


## With Individual Level Data

We create three data sets to illustrate the scenarios where either trivariate or bivariate data is available.
...
If we have from bivariate data sources, i.e. one data set contains observations on $(X, Z)$, and the other observations on $(Y,Z)$, we can still find bounds on ACE.
